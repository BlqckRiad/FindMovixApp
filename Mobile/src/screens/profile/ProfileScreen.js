...
import { generateVerificationCode, saveVerificationCode, verifyCode, getEmailTemplate } from '../../utils/emailVerification';
...
const ProfileScreen = ({ navigation }) => {
    ...
    const loadUserProfile = async () => {
        try {
            ...
            const response = await axios.get(`https://apiuser.findmovix.com/api/User/GetKisiWithID?id=${userID}`);
    ...
    const handleUpdateProfile = async (values) => {
        try {
            ...
            await axios.post('https://apiuser.findmovix.com/api/UserUpdate/UserUpdate', updateData);
    ...
    const requestPermissions = async (type) => {
        if (type === 'camera') {
            ...
            if (status !== 'granted') {
                Alert.alert(
                    'İzin Gerekli',
                    'Kamera erişimi için izin gereklidir.',
                    [{ text: t.ok }]
                );
                return false;
            }
    ...
    const uploadPhoto = async (uri) => {
        try {
            ...
            if (!allowedExtensions.includes(extension)) {
                Alert.alert('Hata', 'Yalnızca resim dosyaları (.jpg, .jpeg, .png, .gif, .bmp) yüklenebilir.');
                return;
            }
            ...
            const imageResponse = await axios.post('https://apiimage.findmovix.com/api/Image/AddImage', formData, {
                headers: {
            ...
            if (imageResponse.data) {
                ...
                await axios.post('https://apiuser.findmovix.com/api/UserUpdate/UserImageUpdate', updateImageData);
    ...
    const handleEmailUpdate = async (newEmail) => {
        try {
            ...
            await axios.post('https://apiuser.findmovix.com/api/Notification/SendEmail', emailData);
    ...
    const handleVerifyEmail = async (code) => {
        ...
        try {
            ...
            const response = await axios.post('https://apiuser.findmovix.com/api/UserUpdate/UserEmailUpdate', updateData);
            ...
            <ScrollView style={styles.content}>
                ...
                <View style={styles.section}>
                    ...
                    <ProfileItem
                        ...
                        label={t.birthDate}
                        value={userData?.userDate ? new Date(userData.userDate).toLocaleDateString() : t.notSpecified}
                    ...
                    <ProfileItem
                        ...
                        label={t.gender}
                        value={userData?.userSexsID === 1 ? t.male : userData?.userSexsID === 2 ? t.female : t.notSpecified}
